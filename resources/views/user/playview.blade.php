...
@section('meta')
    @if($quiz->metadata)
        <meta name="description" content="{{ $quiz->metadata }}">
    @endif
...
.social-share-fixed {
    ...
    box-shadow: 0px -2px 6px rgba(0, 0, 0, 0.1); /* Add shadow for separation */
    z-index: 1000;
    gap: 15px;
}
...
@section('og:description', "Take this quiz: {$quiz->name} and see how you score!")
@section('og:image', asset('storage/' . $quiz->image))
...
<div class="social-share-fixed">
    ...
    <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($quiz->name) }}" 
        target="_blank" class="social-icon twitter" title="Share on Twitter">
        <i class="fab fa-twitter"></i>
    </a>
    <a href="https://www.linkedin.com/shareArticle?mini=true&url={{ urlencode(url()->current()) }}&title={{ urlencode($quiz->name) }}" 
        target="_blank" class="social-icon linkedin" title="Share on LinkedIn">
        <i class="fab fa-linkedin-in"></i>
    </a>
    <a href="https://api.whatsapp.com/send?text={{ urlencode($quiz->name) }}%20{{ urlencode(url()->current()) }}" 
